...
@section('content')
    ...
    <div class="bg-white shadow-xl rounded-[12px] p-7 space-y-5 border border-gray-300">
        <div class="border-b-2 border-gray-800 pb-2">
            <h2 class="text-xl font-semibold">Ubah Data Detail Pengumpulan</h2>
            <h2 class="text-xl font-semibold">Periode {{$data->pengumpulan->bulan}}/{{$data->pengumpulan->tahun}}</h2>
        </div>

        <form action="{{ route('pengumpulan.detail.update', [$id, $detail_id]) }}" class="space-y-6" method="POST"
              enctype="multipart/form-data">
            @csrf
            <div class="grid grid-cols-2 gap-5">
                <div class="space-y-2 col-span-2">
                    <p>Periode <label class="text-red-700">*</label></p>
                    <input readonly type="text" value="{{ $data->pengumpulan->bulan }}/{{$data->pengumpulan->tahun}}"
                ...
                <div class="space-y-2">
                    <p>NPWZ <label class="text-red-700">*</label></p>
                    <input id="npwz" readonly value="{{$data->muzakki->npwz}}" type="text"
                           class="w-full bg-gray-300 rounded-[12px] py-2 px-4">
                </div>

                <div class="space-y-2">
                    <p>Nama Muzakki <label class="text-red-700">*</label></p>
                    <input id="nama" readonly value="{{$data->muzakki->user->nama}}" type="text"
                           class="w-full bg-gray-300 rounded-[12px] py-2 px-4">
                </div>

                <div class="space-y-2">
                    <p>Jenis Muzakki <label class="text-red-700">*</label></p>
                    <input id="jenis" readonly value="{{$data->muzakki->jenis}}" type="text"
                           class="w-full bg-gray-300 rounded-[12px] py-2 px-4">
                </div>

                <div class="space-y-2">
                    <p>Jenis Dana <label class="text-red-700">*</label></p>
                    <select name="jenis_dana" class="w-full bg-[#F6F8FA] rounded-[12px] py-2 px-4">
                        <option value="{{$data->jenis_dana}}" selected>{{ucwords($data->jenis_dana)}}</option>
                        <option disabled>===================</option>
                        @foreach($jenis_dana as $index => $jd)
                            <option value="{{$index}}">{{ucwords($jd)}}</option>
                        @endforeach
                    </select>
                    @error('jenis_dana')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                ...
                <div class="space-y-2">
                    <p>Dalam Neraca <label class="text-red-700">*</label></p>
                    <select name="dalam_neraca" class="w-full bg-[#F6F8FA] rounded-[12px] py-2 px-4">
                        <option selected
                                value="{{$data->dalam_neraca}}">{{$data->dalam_neraca ? 'Ya' : 'Tidak'}}</option>
                        <option disabled>===================</option>
                ...
                <div class="space-y-2">
                    ...
                    <select onchange="toggleVia()" name="via" class="w-full bg-[#F6F8FA] rounded-[12px] py-2 px-4">
                        <option selected value="{{$data->via}}">{{ucwords($data->via)}}</option>
                        <option disabled>===================</option>
                ...
                <div class="bank space-y-2 hidden">
                    <p>Pilih Bank <label class="text-red-700">*</label></p>
                    <select name="rekening_id" class="w-full bg-[#F6F8FA] rounded-[12px] py-2 px-4">
                        <option value="{{$data->rekening_id}}">{{$data->rekening->bank}} - {{$data->rekening->no_rek}}
                            - {{$data->rekening->pemilik_rekening}}</option>
                ...
                <div class="space-y-2 col-span-2">
                    <p>Upload Bukti Pembayaran <label class="text-red-700">(Kosongkan jika tidak ingin mengubah)</label>
                    </p>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $('#search').on('keyup', function () {
                ...
                if (query !== '') {
                    $.ajax({
                        url: '/dashboard/user/muzakki/json/' + query,
                        method: 'GET',
                        data: {
                            query: query
                        },
                        success: function (data) {
                            $('select[name="muzakki_id"]').empty();
                            $('select[name="muzakki_id"]').append(
            ...
            $('select[name="muzakki_id"]').change(function () {
                var selectedMustahiqId = $(this).val();

                $.ajax({
                    url: '/dashboard/user/muzakki/json/detail/' + selectedMustahiqId,
                    method: 'GET',
                    success: function (data) {
                        $('#npwz').val(data.npwz);
