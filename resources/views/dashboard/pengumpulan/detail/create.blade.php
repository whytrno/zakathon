...
@section('content')
    ...
    <div class="bg-white shadow-xl rounded-[12px] p-7 space-y-5 border border-gray-300">
        <div class="border-b-2 border-gray-800 pb-2">
            <h2 class="text-xl font-semibold">Tambah Data Detail Pengumpulan</h2>
            <h2 class="text-xl font-semibold">Periode {{$data->bulan}}/{{$data->tahun}}</h2>
        </div>

        <form action="{{ route('pengumpulan.detail.store', $id) }}" class="space-y-6" method="POST"
              enctype="multipart/form-data">
            @csrf
            <div class="grid grid-cols-2 gap-5">
                <div class="space-y-2 col-span-2">
                    <p>Periode <label class="text-red-700">*</label></p>
                    <input readonly type="text" value="{{ $data->bulan }}/{{$data->tahun}}"
                ...
                <div class="space-y-2">
                    <p>Cari Muzakki (Nama atau NPWZ) <label class="text-red-700">*</label></p>
                    <input type="text" id="search" class="w-full bg-[#F6F8FA] rounded-[12px] py-2 px-4"
                           placeholder="Cari NPWZ atau Nama">
                </div>

                <div class="space-y-2">
                    <p>Pilih Muzakki <label class="text-red-700">*</label></p>
                    <select name="muzakki_id" class="w-full bg-[#F6F8FA] rounded-[12px] py-2 px-4">
                        <option value=""></option>
                    </select>
                    @error('muzakki_id')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                ...
                <div class="space-y-2">
                    ...
                    <input id="npwz" readonly type="text" class="w-full bg-gray-300 rounded-[12px] py-2 px-4">
                </div>
