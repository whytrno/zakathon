...
@section('content')
    ...
    <div class="bg-white shadow-xl rounded-[12px] p-7 space-y-5 border border-gray-300">
        <div class="border-b-2 border-gray-800 flex justify-between pb-2">
            <h2 class="text-xl font-semibold">Ubah Data Program Donasi</h2>
        </div>

        <form action="{{ route('donasi.update', $id) }}" class="space-y-6" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="grid grid-cols-2 gap-5">
                ...
                <div class="space-y-2 col-span-2">
                    ...
                    <input type="number" value="{{ $data->target_donasi }}" name="target_donasi"
                           class="w-full bg-[#F6F8FA] rounded-[12px] py-2 px-4">
                    @error('target_donasi')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
